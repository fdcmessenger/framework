...
public abstract class BaseTreeManagerImpl<T extends BaseTreeNode, PK extends Serializable> extends
		BaseManagerImpl<T, PK> implements BaseTreeManager<T, PK> {
	BaseTreeDao treeDao;
	...
	public T insertNode(T node) {
		...
		/*
		 * BaseTreeNode parent = node.getParentNode(); if (parent != null) {
		 * parent = this.get((PK) parent.getId());
		 * treeDao.increaseleftAndRight(parent.getLft());
		 * parent.setRgt(parent.getRgt() + 2); node.setLft(parent.getLft() + 1);
		 * node.setRgt(parent.getLft() + 2); node.setLevel(parent.getLevel() +
		 * 1); return (T) treeDao.insertNode(node); } else { // do nothing
		 * node.setRgt(node.getLft() + 1); return (T) treeDao.insertNode(node);
		 * }
		 */
