...
jQuery(document).ready(function(){
    ...
    $("#newPermission").click(function(event){
        ...
        $toNewPermission.append($name.val() + '(' + $shortName.val() + ')' + '<input type="checkbox" name="model.permissions[' + maxId + '].shortName" value="' + $shortName.val() + '" checked="checked" rel="' + maxId + '"/>&nbsp;&nbsp;'); 
        $toNewPermission.append('<input type="hidden" name="model.permissions[' + maxId + '].name" value="' + $name.val() + '" rel="' + maxId + '"/>');
        $toNewPermission.append('<input type="hidden" name="model.permissions[' + maxId + '].description" value="' + $description.val() + '" rel="' + maxId + '"/>');
...
<form id="permissionForm" method="post" action="${ctx }/management/security/module/update" class="required-validate pageForm" onsubmit="">
	...
	<div id="permissionFormContent" class="pageFormContent" layoutH="58">
		...
		<p>
			<label>名称：</label>
			<input type="text" name="model.name" class="validate[required,maxSize[32]] required" size="32" maxlength="32" value="${model.name }" alt="请输入模块名称"/>
		</p>	
		<p>
			<label>优先级：</label>
			<input type="text" name="model.priority" class="validate[required,custom[integer],min[1],max[99]] required" size="2" maxlength="2" value="${model.priority }"/>
			<span class="info">&nbsp;&nbsp;默认:99</span>
		</p>		
		<p>
			<label>URL：</label>
			<input type="text" name="model.url" class="validate[required,maxSize[255]] required" size="32" maxlength="255" alt="以/或者http开头" value="${model.url }"/>
		</p>		
		<p>
			<label>授权名称：</label>
			<input type="text" name="model.sn" class="validate[required,maxSize[32]] required" size="32" maxlength="32" alt="授权名称" value="${model.sn }" readOnly=readOnly/>
		</p>		
		<p>
			<label>描述：</label>
			<input type="text" name="model.description" class="validate[maxSize[255]]" size="32" maxlength="255" alt="描述" value="${model.description }"/>
		</p>
		<p>
			<label>父模块：</label>
			...
			<a class="btnLook" href="${ctx}/security/lookupModule" lookupGroup="model.parentNode" mask="true" title="更改父模块" width="400">查找带回</a>			
		</p>
		</fieldset>
		<fieldset>
		<legend>自定义授权</legend>
			...
			${p.name}(${p.shortName})<input type="checkbox" name="model.permissions[${s.index}].shortName" value="${p.shortName}" checked="checked" rel="${s.index}"/>&nbsp;&nbsp;
