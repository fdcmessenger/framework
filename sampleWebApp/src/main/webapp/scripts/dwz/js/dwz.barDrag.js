	...
	$.fn.jBar = function(options){
		...
		return this.each(function(){
			...
			$(op.toggleBut, bar).click(function(){
			   // console.log("1");
				...
				$(op.container).animate({left: cleft,width: cwidth},50,function(){
					bar.animate({left: barleft}, 0, function(){
						bar.hide();
						sbar.show().css("left", -50).animate({left: 5}, 0);
					});
				...
				$(op.collapse,sbar).click(function(){
				   // console.log("1.1");
					...
					if(bar.is(":hidden")) {
						...
						bar.show().animate({left: sbarwidth}, 0);
			...
			$(op.toggleBut, sbar).click(function(){
			    //console.log("2");
				DWZ.ui.sbar = true;
				sbar.animate({left: -25}, 0, function(){				
					bar.show();
				});
				bar.animate({left: 5}, 0, function(){
					$(op.splitBar).show();
